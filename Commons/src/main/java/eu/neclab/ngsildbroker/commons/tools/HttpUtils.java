...
//			.entity(new NGSIRestResponse(ErrorType.NotFound, "Resource not found.").toJson()).build();
	...
	public static Uni<MultiMap> getAdditionalHeaders(Map<String, Object> registration, List<Object> context,
			List<String> accept, WebClient webClient) {
		...
		if (receiverInfo != null) {
			...
			return JsonLdProcessor.compact(receiverInfo, context, opts, webClient).onItem().transform(headerMap -> {
...
//	}
	...
	public static Uni<RestResponse<Object>> generateEntityResult(List<Object> contextHeader, Context context,
		...
		return generateCompactedResult(contextHeader, context, acceptHeader, entity, geometryProperty, options,
				langQuery, false, webClient).onItem().transform(resultBodyAndHeaders -> {
	...
	public static Uni<Tuple2<Object, List<Tuple2<String, String>>>> generateCompactedResult(List<Object> contextHeader,
			...
			LanguageQueryTerm langQuery, boolean forceArray, WebClient webClient) {
		...
		case 1:
			uni = JsonLdProcessor.compact(entity, contextHeader, context, opts, -1, optionSet, langQuery, webClient)
		...
		case 2:
			...
			break;
		case 3:
			uni = JsonLdProcessor.toRDF(entity, webClient).onItem().transform(rdf -> {
							...
							return Uni.createFrom()
									.item(Tuple3.of(
											JsonUtils.toPrettyString(
													generateGeoJson(compacted, geometryProperty, contextHeader)),
	...
	public static Uni<Context> getContextFromPayload(Map<String, Object> originalPayload, List<Object> atContextHeader,
		...
		if (payloadAtContext == null) {
			...
			if (atContextHeader.isEmpty()) {
				return Uni.createFrom().item(JsonLdProcessor.getCoreContext());
			} else {
				return JsonLdProcessor.getCoreContextClone().parse(atContextHeader, true, webClient);
			}
			...
			return JsonLdProcessor.getCoreContextClone().parse(payloadAtContext, true, webClient);
	...
	public static Uni<Tuple2<Context, Map<String, Object>>> expandBody(HttpServerRequest request, String payload,
			int payloadType, WebClient webClient) {
		...
		return expandBody(request, originalPayload, payloadType, webClient);
	}

	public static Uni<Tuple2<Context, Map<String, Object>>> expandBody(HttpServerRequest request,
			Map<String, Object> originalPayload, int payloadType, WebClient webClient) {
		...
		return HttpUtils.getContextFromPayload(originalPayload, atContext, atContextAllowed, webClient).onItem()
				.transformToUni(context -> {
					return JsonLdProcessor
							.expand(context, originalPayload, opts, payloadType, atContextAllowed, webClient).onItem()
							.transform(list -> {
	...
	public static Uni<RestResponse<Object>> generateQueryResult(HttpServerRequest request, QueryResult queryResult,
			...
			Context context, WebClient webClient) {
		...
		return generateCompactedResult(atContext, context, acceptHeader, queryResult.getData(), geometryProperty,
				options, lang, true, webClient).onItem().transform(resultAndHeaders -> {
					String nextLink;
					String prevLink;
					if (request != null) {
	...
	public static Uni<Context> getContext(List<Object> headerContext, WebClient webClient) {
		Context context;
		if (headerContext.isEmpty()) {
			return Uni.createFrom().item(JsonLdProcessor.getCoreContext());
		} else {
			return JsonLdProcessor.getCoreContextClone().parse(headerContext, false, webClient);
		}
