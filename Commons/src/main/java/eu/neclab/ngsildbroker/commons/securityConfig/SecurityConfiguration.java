 ...
 * 
 * @Value("${security.active:false}") boolean securityActive;
 * 
 * 
 * 
 * @Bean public SecurityWebFilterChain configure(ServerHttpSecurity http) throws
 * Exception { System.err.println(
 * "%%%%%%%%%%%%%%%^^^^^^^^^^^^^&&&&&&&&&&&&&&&***************"); http.cors();
 * if (securityActive) { //http.oauth2ResourceServer().jwt();
 * 
 * 
 * } else { http.authorizeExchange().anyExchange().permitAll(); } return
 * http.build(); } }
 */