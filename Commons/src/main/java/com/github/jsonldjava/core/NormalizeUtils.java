...
class NormalizeUtils {
					...
					for (int pgi = 0;; pgi++) {
						if (pgi == hashes.size()) {
							...
							for (int cai = 0; cai < quads.size(); ++cai) {
								final Map<String, Object> quad = (Map<String, Object>) quads.get(cai);
								for (final String attr : new String[] { "subject", "object", "name" }) {
									if (quad.containsKey(attr)) {
										final Map<String, Object> qa = (Map<String, Object>) quad.get(attr);
	...
	/**
	 * Produces a hash for the paths of adjacent bnodes for a bnode, incorporating
	 * all information about its subgraph of bnodes. This method will recursively
	 * pick adjacent bnode permutations that produce the lexicographically-least
	 * 'path' serializations.
	 *
	...
	private static HashResult hashPaths(String id, Map<String, Object> bnodes, UniqueNamer namer,
		...
		try {
			...
			final List<Object> quads = (List<Object>) ((Map<String, Object>) bnodes.get(id)).get("quads");
					...
					for (int hgi = 0;; hgi++) {
						...
						while (true) {
							...
							for (int nrn = 0;; nrn++) {
								...
								final HashResult result = hashPaths(bnode, bnodes, namer, pathNamerCopy);
	...
	private static String hashQuads(String id, Map<String, Object> bnodes, UniqueNamer namer) {
		...
		final List<Map<String, Object>> quads = (List<Map<String, Object>>) ((Map<String, Object>) bnodes.get(id))
				.get("quads");
	 ...
	 * (subject or object). If the node is a blank node and its value does not match
	 * the given blank node ID, it will be returned.
	 *
	...
	private static String getAdjacentBlankNodeName(Map<String, Object> node, String id) {
		return "blank node".equals(node.get("type"))
				&& (!node.containsKey("value") || !Obj.equals(node.get("value"), id)) ? (String) node.get("value")
						: null;
	}
