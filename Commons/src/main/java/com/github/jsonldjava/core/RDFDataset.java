...
public class RDFDataset extends LinkedHashMap<String, Object> {
	...
	public static class Quad extends LinkedHashMap<String, Object> implements Comparable<Quad> {
		...
		public Quad(final String subject, final String predicate, final String value, final String datatype,
				final String language, final String graph) {
		...
		private Quad(final String subject, final String predicate, final Node object, final String graph) {
			this(subject.startsWith("_:") ? new BlankNode(subject) : new IRI(subject), new IRI(predicate), object,
					graph);
		};
	 ...
	 * @param datatype  the datatype of the literal object for the triple (null
	 *                  values will default to xsd:string)
	 * @param language  the language of the literal object for the triple (or null)
	 */
	public void addTriple(final String subject, final String predicate, final String value, final String datatype,
	 ...
	 * @param graph     the graph to add this triple to
	 */
	public void addQuad(final String subject, final String predicate, final String object, String graph) {
		if (graph == null) {
			graph = "@default";
		}
		if (!containsKey(graph)) {
		...
		for (final String id : subjects) {
			...
			for (String property : properties) {
				...
				for (final Object item : values) {
					...
					if (isList(item)) {
						final List<Object> list = (List<Object>) ((Map<String, Object>) item).get("@list");
						Node last = null;
						Node firstBNode = nil;
						if (!list.isEmpty()) {
	...
	/**
	 * Converts a JSON-LD value object to an RDF literal or a JSON-LD string or node
	 * object to an RDF resource.
	 *
	...
	private Node objectToRDF(Object item) {
		...
		if (isValue(item)) {
			...
			if (value instanceof Boolean || value instanceof Number) {
				...
				return new Literal((String) value, datatype == null ? RDF_LANGSTRING : (String) datatype,
