 ...
 * eu.neclab.ngsildbroker.commons.datatypes.QueryResult; import
 ...
 * eu.neclab.ngsildbroker.queryhandler.services.QueryService;
 * 
 * @SpringBootTest(properties = {
 * "spring.main.allow-bean-definition-overriding=true" })
 * 
 * @RunWith(PowerMockRunner.class) //@WebMvcTest(secure = false)
 * 
 ...
 * "javax.xml.*", "org.xml.*", "org.w3c.*", "com.sun.org.apache.xalan.*",
 * "javax.activation.*", "javax.net.*", "javax.security.*" })
 * 
 * @SuppressWarnings("unchecked") // No need in tests system should fail by
 * itself on wrong data types... makes // life easier with any() public class
 * QueryControllerTest {
 * 
 ...
 * 
 * @Autowired ParamsResolver paramsResolver; // @InjectMocks // @Spy //
 * QueryController qc;
 * 
 * private String entity; private String response = ""; //private String
 * linkHeader =
 ...
 * 
 * @Before public void setup() { // PowerMockito.mockStatic(HttpUtils.class);
 * MockitoAnnotations.initMocks(this); // this.mockMvc =
 * MockMvcBuilders.standaloneSetup(qc).build(); // @formatter:off entity =
 ...
 * // @formatter:on }
 * 
 * @After public void tearDown() { entity = null; response = null; }
 * 
 ...
 * .andExpect(redirectedUrl(linkHeader)).andDo(print()); verify(queryService,
 * times(1)).getData(any(), any(), any(), any(), any(), any(), any(),
 * any(),any(),any(),any()); // verify(qc, times(1)).generateReply(any(), any(),
 * any());
 * 
 ...
 * 
 * QueryResult result = new QueryResult(entities, null, ErrorType.None, -1,
 * true); // when(HttpUtils.parseLinkHeader(any(HttpServletRequest.class), //
 * NGSIConstants.HEADER_REL_LDCONTEXT)).thenReturn(linkHeaders); //
 * when(paramsResolver.resolveQueryLdContext(any(), //
 * any())).thenReturn(resolveQueryLdContext); //
 * when(paramsResolver.getQueryParamsFromUriQuery(any(),any())).thenReturn(new
 * // QueryParams().withAttrs("brandName")); // when(queryService.getData(any(),
 * any(), any(), any(), null, null, // null)).thenReturn(result);
 * 
 * // Mockito.doReturn(resolveQueryLdContext).when(paramsResolver).
 * resolveQueryLdContext(any(), any()); // Mockito.doReturn(new
