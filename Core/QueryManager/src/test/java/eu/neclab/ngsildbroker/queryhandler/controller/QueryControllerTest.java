...
//			Mockito.doReturn(responseEntity).when(qc).generateReply(any(), any(), any());
			...
			Mockito.doReturn(result).when(queryService).getData(any(), any(), any(), any(), any(), any(), any(), any(),
					any());
			mockMvc.perform(get("/ngsi-ld/v1/entities/{entityId}", "urn:ngsi-ld:Vehicle:A100")
					.accept(AppConstants.NGB_APPLICATION_JSON)).andExpect(status().isOk())
					.andExpect(jsonPath("$.id").value("urn:ngsi-ld:Vehicle:A100"));
//					.andExpect(redirectedUrl(linkHeader)).andDo(print());
			verify(queryService, times(1)).getData(any(), any(), any(), any(), any(), any(), any(), any(), any());
//			verify(qc, times(1)).generateReply(any(), any(), any());
	...
	public void getEntityNotFoundTest() {
		try {
			...
			// true);
			Mockito.doThrow(new ResponseException(ErrorType.NotFound)).when(queryService).getData(any(), any(), any(),
...
//			Mockito.doReturn(responseEntity).when(qc).generateReply(any(), any(), any());
			...
			Mockito.verify(queryService, times(1)).getData(any(), any(), any(), any(), any(), any(), any(), any(),
					any());
//			Mockito.verify(qc, times(1)).generateReply(any(), any(), any());
	...
	public void serviceThrowsAlreadyExistsTest() {
		...
		try {
			...
			Mockito.doThrow(new ResponseException(ErrorType.BadRequestData)).when(queryService).getData(any(), any(),
