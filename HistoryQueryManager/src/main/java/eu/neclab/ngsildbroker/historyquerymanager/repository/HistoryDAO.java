...
public class HistoryDAO {
    ...
    public Uni<Map<String, Object>> retrieveEntity(String tenant, String entityId, AttrsQueryTerm attrsQuery,
        ...
        return clientManager.getClient(tenant, false).onItem().transformToUni(client -> {
            ...
            if (attrsQuery != null) {
                ...
                for (Object attr : attrsQuery.getAttrs().toArray()) {
                    sql.append("$").append(dollarCount).append(",");
                    dollarCount++;
                    tuple.addString((String) attr);
                }
    ...
    public Uni<QueryResult> query(String tenant, String[] entityIds, TypeQueryTerm typeQuery, String idPattern,
        ...
        return clientManager.getClient(tenant, false).onItem().transformToUni(client -> {
            ...
            sql.append(" from attributeData left join ").append(entityInfos).append(" on ").append(entityInfos).append(".id = attributeData.id group by attributeData.id, ").append(entityInfos).append(".e_types, ").append(entityInfos).append(".r_createdat, ").append(entityInfos).append(".r_modifiedat, ").append(entityInfos).append(".r_deletedat, ").append(entityInfos).append(".scopes");
            sql.append(" LIMIT $");
            sql.append(dollarCount);
            dollarCount++;
            sql.append(" OFFSET $");
