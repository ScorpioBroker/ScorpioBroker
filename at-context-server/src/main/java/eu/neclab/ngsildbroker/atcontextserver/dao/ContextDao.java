...
public class ContextDao {
	...
	public Uni<RestResponse<Object>> getById(String id, Boolean details) {
		...
		return pgPool.preparedQuery(sql).execute(Tuple.of(id)).onItem().transformToUni(rows -> {
			if (rows.size() > 0) {
	...
	public Uni<List<Object>> getAllContexts(String kind, Boolean details) {
		...
		return pgPool.preparedQuery(sql).execute().onItem().transform(rows -> {
			rows.forEach(i -> {
				...
				.transform((rows, failure) -> {
					if (failure != null) {
						if (failure instanceof PgException && ((PgException) failure).getCode().equals("23505")) {
							...
							return RestResponse.status(RestResponse.Status.INTERNAL_SERVER_ERROR, failure.getMessage());
						}
