...
public class QueryController {
        ...
        return HttpUtils.getAtContext(request).onItem().transformToUni(headerContext -> {
            ...
            return queryService
                    ...
                    .transform(entity -> HttpUtils.generateEntityResult(headerContext, context, acceptHeader, entity,
                            geometryProperty, options)).onFailure().recoverWithItem(HttpUtils::handleControllerExceptions);
        });
    }
    ...
    public Uni<RestResponse<Object>> query(HttpServerRequest request, @QueryParam("id") Set<String> id,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToUni(headerContext -> {
            ...
            return queryService.query(headers, id, typeQueryTerm, idPattern, attrsQueryTerm, qQueryTerm, csfQueryTerm, geoQueryTerm,
                    scopeQueryTerm, lang, limit.get(), offset, count, localOnly).onItem().transform(t -> {
    ...
    public Uni<RestResponse<Object>> getAllTypes(HttpServerRequest request,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToUni(headerContext -> {
            ...
            if (details) {
                return queryService.getTypesWithDetail(HttpUtils.getHeaders(request), localOnly).onItem().transform(types -> {
                    if (types.isEmpty()) {
                        return RestResponse.notFound();
                    } else return RestResponse.ok(types);
                });
            } else return queryService.getTypes(HttpUtils.getHeaders(request), localOnly).onItem().transform(types -> {
                if (types.isEmpty()) {
                    return RestResponse.notFound();
                } else return RestResponse.ok(types);
            });
    ...
    public Uni<RestResponse<Object>> getType(HttpServerRequest request, @PathParam("entityType") String type,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToUni(headerContext -> {
            ...
            return queryService.getType(HttpUtils.getHeaders(request), type, localOnly).onItem().transform(
                    map -> {
                        if (map.isEmpty()) {
                            return RestResponse.notFound();
                        } else return RestResponse.ok(map);
                    }
            );
        });
    }
    ...
    public Uni<RestResponse<Object>> getAllAttributes(HttpServerRequest request,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToUni(headerContext -> {
            ...
            return queryService.getAttribs(HttpUtils.getHeaders(request), localOnly).onItem().transform(
                    map -> {
                        if (map.isEmpty()) {
                            return RestResponse.notFound();
                        } else return RestResponse.ok(map);
    ...
    public Uni<RestResponse<Object>> getAttribute(HttpServerRequest request, @PathParam("attribute") String attribute,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToUni(headerContext -> {
            ...
            return queryService.getAttrib(HttpUtils.getHeaders(request), attribute, localOnly).onItem().transform(
                    map -> {
                        if (map.isEmpty()) {
                            return RestResponse.notFound();
                        } else return RestResponse.ok(map);
