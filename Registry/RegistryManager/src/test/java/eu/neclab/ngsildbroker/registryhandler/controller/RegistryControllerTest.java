...
public class RegistryControllerTest {
  ...
  updatePayload = "{\r\n" + "	\"id\": \"urn:ngsi-ld:ContextSourceRegistration:csr1a3458\",\r\n"
	...
	public void registerCSourceTest() {
		try {
			when(csourceService.createEntry(any(),any())).thenReturn("urn:ngsi-ld:ContextSourceRegistration:csr1a3458");
			ResultActions resultAction=	mockMvc.perform(post("/ngsi-ld/v1/csourceRegistrations/").contentType(AppConstants.NGB_APPLICATION_JSONLD).content(payload))
	...
	public void registerCSourceAlreadyExistTest() {
		try {
			when(csourceService.createEntry(any(),any())).thenThrow(new ResponseException(ErrorType.AlreadyExists,"urn:ngsi-ld:ContextSourceRegistration:csr1a3458"));
			ResultActions resultAction=	mockMvc.perform(post("/ngsi-ld/v1/csourceRegistrations/").contentType(AppConstants.NGB_APPLICATION_JSONLD).content(payload))
					.andExpect(status().isConflict()).andExpect(jsonPath("$.title").value("Already exists.")).andDo(print());
	...
	public void registerCSourceBadRequestTest() {
		try {
			when(csourceService.createEntry(any(),any())).thenThrow(new ResponseException(ErrorType.BadRequestData,"You have to provide a valid payload"));
			ResultActions resultAction=	mockMvc.perform(post("/ngsi-ld/v1/csourceRegistrations/").contentType(AppConstants.NGB_APPLICATION_JSONLD).content(payload))
					.andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request Data.")).andDo(print());
   ...
   /**
	...
	public void deleteCsourceTest() {
		...
		ResultActions resultAction =	mockMvc.perform(delete("/ngsi-ld/v1/csourceRegistrations/{registrationId}", "urn:ngsi-ld:ContextSourceRegistration:csr1a3458")
	...
	public void updateCSourceTest() {
		...
		ResultActions resultAction =	mockMvc.perform(patch("/ngsi-ld/v1/csourceRegistrations/{registrationId}", "urn:ngsi-ld:ContextSourceRegistration:csr1a3458")
	...
	public void updateCSourceNotFoundTest() {
		try {
		   UpdateResult result = mock(UpdateResult.class);
			when(csourceService.appendToEntry(any(), any(), any(), any())).thenThrow(new ResponseException(ErrorType.NotFound,"Resource not found."));
		ResultActions resultAction =	mockMvc.perform(patch("/ngsi-ld/v1/csourceRegistrations/{registrationId}", "urn:ngsi-ld:ContextSourceRegistration:csr1a3458")
					.contentType(AppConstants.NGB_APPLICATION_JSONLD).content(updatePayload))
					.andExpect(status().isNotFound()).andExpect(jsonPath("$.title").value("Resource not found.")).andDo(print());
			MvcResult mvcResult = resultAction.andReturn();
			MockHttpServletResponse res = mvcResult.getResponse();
			int status = res.getStatus();
			assertEquals(404, status);
	...
	public void updateCSourceBadRequestTest() {
		try {
		   UpdateResult result = mock(UpdateResult.class);
			when(csourceService.appendToEntry(any(), any(), any(), any())).thenThrow(new ResponseException(ErrorType.BadRequestData,"Bad Request Data."));
		ResultActions resultAction =	mockMvc.perform(patch("/ngsi-ld/v1/csourceRegistrations/{registrationId}", "urn:ngsi-ld:ContextSourceRegistration:csr1a3458")
					.contentType(AppConstants.NGB_APPLICATION_JSONLD).content(updatePayload))
					.andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request Data."));
			MvcResult mvcResult = resultAction.andReturn();
	...
	public void getCSourceByIdTest() throws Exception {
	   ...
	   when(csourceService.getCSourceRegistrationById(any(),any())).thenReturn(updatePayload);
		ResultActions resultAction = mockMvc.perform(
				get("/ngsi-ld/v1/csourceRegistrations/{registrationId}/","urn:ngsi-ld:ContextSourceRegistration:csr1a3458")
	...
	public void getCSourceByIdNotFoundTest() throws Exception {
	   ...
	   when(csourceService.getCSourceRegistrationById(any(),any())).thenThrow(new ResponseException(ErrorType.NotFound,"Resource not found."));
		ResultActions resultAction = mockMvc.perform(
				get("/ngsi-ld/v1/csourceRegistrations/{registrationId}/","urn:ngsi-ld:ContextSourceRegistration:csr1a3451")
				.accept(AppConstants.NGB_APPLICATION_JSON))
	...
	public void getCSourceByIdBadRequestTest() throws Exception {
	   ...
	   when(csourceService.getCSourceRegistrationById(any(),any())).thenThrow(new ResponseException(ErrorType.BadRequestData,"Bad Request Data."));
		ResultActions resultAction = mockMvc.perform(
				get("/ngsi-ld/v1/csourceRegistrations/{registrationId}/","urn:ngsi-ld:ContextSourceRegistration:csr1a3451")
				.accept(AppConstants.NGB_APPLICATION_JSON))
	...
	public void getDiscoverCSourceTest() throws Exception {
		...
		ResultActions resultAction = mockMvc.perform(
				get("/ngsi-ld/v1/csourceRegistrations?type=Test1")
				.accept(AppConstants.NGB_APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void getDiscoverCSourceBadRequestTest() throws Exception {
		...
		QueryResult result = mock(QueryResult.class);
		when(csourceService.getData(any(), any(), any(), any(), any())).thenThrow(new ResponseException(ErrorType.BadRequestData,"Bad Request Data."));
	...
	public void getDiscoverCSourceNotFoundTest() throws Exception {
		...
		QueryResult result = mock(QueryResult.class);
		when(csourceService.getData(any(), any(), any(), any(), any())).thenThrow(new ResponseException(ErrorType.NotFound,"Resource not found."));
