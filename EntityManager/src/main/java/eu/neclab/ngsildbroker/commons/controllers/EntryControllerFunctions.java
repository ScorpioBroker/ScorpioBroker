...
public interface EntryControllerFunctions {
    ...
    static Uni<RestResponse<Object>> updateMultiple(EntityService entityService, HttpServerRequest request,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToMulti(t -> {
            ...
            if (!tupleItem.getItem3().containsKey(NGSIConstants.JSON_LD_ID) && !tupleItem.getItem3().containsKey(NGSIConstants.QUERY_PARAMETER_ID)) {
            ...
            return entityService.updateMultipleEntry(HttpUtils.getTenant(request), tuple3s)
    ...
    static Uni<RestResponse<Object>> createMultiple(EntityService entityService, HttpServerRequest request,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToMulti(t -> {
            ...
            /// context
            return entityService.createMultipleEntry(tenant, results).onItem().transform(HttpUtils::generateBatchResult);
        });
    }
    ...
    static Uni<RestResponse<Object>> deleteMultiple(EntityService entityService, HttpServerRequest request,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToMulti(t -> {
                ...
                }).collect().asList().onItemOrFailure()
                .transformToUni((ids, fail) ->
                        entityService.deleteMultipleEntry(HttpUtils.getTenant(request), ids, new BatchInfo(random.nextInt(), ids.size()))
                                .onItem().transform(HttpUtils::generateBatchResult));
    }
    ...
    static Uni<RestResponse<Object>> upsertMultiple(EntityService entityService, HttpServerRequest request,
        ...
        return HttpUtils.getAtContext(request).onItem().transformToMulti(t -> {
            ...
            Context context;
            try {
                context = HttpUtils.getContextFromPayload(tt.getItem3(), tt.getItem2(),
                        tt.getItem1());
            } catch (ResponseException e) {
                return Uni.createFrom().failure(e);
            }
        ...
        }).collectFailures().concatenate().collect().asList().onItem().transformToUni(
                results -> {
                    String tenant = HttpUtils.getTenant(request);
                    ...
                    for (Tuple3<Context, String, Map<String, Object>> tuple : results) {
                        ...
                        } else toBeAppend.add(Tuple3.of(tuple.getItem1(), tuple.getItem3(), true));
                    }
                    if (!toBeDeleted.isEmpty()) {
                        return entityService.deleteMultipleEntry(tenant, toBeDeleted, batchInfo)
                                .onItem().transformToUni(t -> entityService.createMultipleEntry(tenant, toBeCreated))
                                .onItem().transform(HttpUtils::generateBatchResult);
                    }
                    else  {
                        return entityService.updateMultipleEntry(tenant, toBeAppend)
                                .onItem()
