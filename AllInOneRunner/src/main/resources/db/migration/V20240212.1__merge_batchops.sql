...
BEGIN
  ...
  FOR newentity IN SELECT jsonb_array_elements FROM jsonb_array_elements(b) LOOP
	 ...
	 resultObj['success'] = resultObj['success'] || jsonb_build_object('id', newentity->'@id', 'old', previous_entity, 'new', merged_json)::jsonb;
	ELSE
     resultObj['failure'] := resultObj['failure'] || jsonb_object_agg(newentity->'@id'->>0, 'Not Found');
	 CONTINUE;
	END IF;
   -- Iterate through keys in JSONB value
    FOR key, value IN SELECT * FROM JSONB_EACH(newentity)
    LOOP
  ...
  -- Construct a JSONB array containing the original previous_entity and the merged JSON
  RETURN resultObj;
END;
$$ LANGUAGE plpgsql;
